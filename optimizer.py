...
def build_optimizer(config, model, simmim=False, is_pretrain=False):
    ...
    if simmim:
        if is_pretrain:
            ...
            depths = config.MODEL.HVT.DEPTHS if config.MODEL.TYPE == 'HVT' else config.MODEL.HVTV2.DEPTHS
            num_layers = sum(depths)
            get_layer_func = partial(get_HVT_layer, num_layers=num_layers + 2, depths=depths)
...
def get_HVT_layer(name, num_layers, depths):
